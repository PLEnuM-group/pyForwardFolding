...
class BinnedData:
    """
    Represents the binned measured data.

    Args:
        name (str): Name of the BinnedData.
        dskey_model_pairs (List[Tuple[str, Model]]): List of tuples where each tuple contains a dataset key and a model.
        binning (AbstractBinning): The binning used to create the histogram.
    """
    ...
    def evaluate_data(
        ...
        """
        Evaluate the observed data histogram.
        ...
        if self.data_key is None:
            raise ValueError(
                f"No data_key configured for data '{self.name}'."
            )
        ...
        ### get len of first key in binning_variables
        weight_dimensions = len(next(iter(input_variables.values())))
    ...
    def _repr_markdown_(self) -> str:
        """
        Markdown representation of the BinnedData object.
